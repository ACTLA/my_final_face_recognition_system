...
камеры с корректной очисткой ресурсов.

Основные функции:
- Инициализация и конфигурация веб-камеры
- Управление состоянием (запуск/остановка)
- Захват кадров в режиме реального времени
- Автоматическое освобождение ресурсов

Архитектурные принципы:
- Поведение одиночки (одна камера на систему)
...
class CameraController:
    ...
    Реализует принцип RAII (получение ресурса - это инициализация)
    ...
    - Корректное завершение с освобождением системных ресурсов
    ...
    def stop_camera(self):
        ...
        Выполняет корректное завершение камеры:
        1. Деактивация флага состояния
        2. Освобождение ресурсов VideoCapture
        3. Очистка объекта захвата
        
        Следует принципу RAII (получение ресурса - это инициализация)
    ...
    def capture_frame(self):
        ...
        Возвращает:
            numpy.ndarray или None: Кадр изображения в формате BGR (Blue-Green-Red)
                                  или None при ошибке захвата или неактивной камере
                                  
        Примечание:
            Возвращаемый кадр имеет формат массива numpy с размерностью (высота, ширина, 3)
