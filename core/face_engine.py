...
- Извлечения биометрических признаков (face embeddings)
...
- HOG (Histogram of Oriented Gradients) для детекции лиц
- CNN (Convolutional Neural Network) для извлечения признаков
...
class FaceAnalysisEngine:
    ...
    - In-memory хранение биометрических отпечатков для быстрого доступа
    - Векторизованные операции сравнения для производительности
    - Настраиваемые пороги уверенности для балансировки точности/полноты
    """
    ...
    def __init__(self):
        ...
        """
        self.registered_user_encodings = []  # Список numpy массивов с кодировками лиц (128-мерные векторы)
    ...
    def load_facial_encodings(self, encodings, user_ids):
        ...
        Args:
            encodings (list): Список numpy массивов размерности (128,) содержащих
    ...
    def generate_facial_encoding(self, image_path):
        ...
        3. Детекция лиц на изображении с помощью HOG детектора
        4. Извлечение 128-мерного вектора признаков лица через CNN
        
        Алгоритм основан на модели ResNet, обученной на датасете лиц
        для создания robust представлений, инвариантных к освещению,
    ...
    def detect_and_recognize_faces(self, frame, scale=0.25):
        ...
        2. Детекция всех лиц на кадре с помощью HOG детектора
            ...
            list: Список словарей с информацией о каждом обнаруженном лице:
                {
                    ...
                    'user_id': str or None,                  # ID распознанного пользователя
    ...
    def draw_detection_rectangle(self, frame, face_info, color, name=""):
        ...
        - Желтый: Ожидание (защита от спама)
