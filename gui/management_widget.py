...
- Transactional операции с автоматическим rollback при ошибках
"""
...
class UserManagementWidget:
    ...
    - CRUD операции с пользователями (Create, Read, Update, Delete)
    - Загрузка и валидация фотографий пользователей
    ...
    - Cascade удаление связанных файлов и данных
    
    Архитектурные особенности:
    - Transactional операции для обеспечения целостности данных
    - Client-side валидация для улучшения UX
    - Safe file operations с proper error handling
    - Event-driven обновление списков после операций
    """
    ...
    def __init__(self, parent_notebook, db, face_engine, load_encodings_callback):
        ...
        Args:
            ...
            load_encodings_callback (callable): Callback для обновления кодировок в системе
        """
        ...
        self.load_encodings_callback = load_encodings_callback  # Callback обновления кодировок
        
        # Состояние виджета
        self.selected_photo_file_path = ""  # Путь к выбранной фотографии
        self.audit_logger_provider = None  # Провайдер системы аудита
    ...
    def set_audit_logger(self, audit_provider_function):
        ...
        Args:
            audit_provider_function (callable): Функция получения экземпляра AuditLogger
        """
    ...
    def initialize_management_interface(self):
        ...
        # Основной контейнер с сеткой для двухпанельного макета
    ...
    def create_user_addition_panel(self, parent):
        ...
        - Секцию загрузки фотографии с превью
    ...
    def create_photo_upload_section(self, parent):
        ...
        # Label для отображения превью фотографии
        ...
        # Индикатор статуса загрузки фотографии
    ...
    def initialize_users_table(self, parent):
        ...
        - Фото: Статус наличия фотографии
    ...
    def handle_photo_selection(self):
        ...
        Открывает диалог выбора файла, валидирует выбранное изображение
        ...
        if file_path:
            ...
            try:
                # Загрузка и масштабирование изображения для превью
                ...
                self.photo_preview.image = photo_preview  # Сохранение ссылки для предотвращения GC
    ...
    def handle_user_addition(self):
        ...
        Использует transactional подход с автоматическим rollback при ошибках.
        """
        ...
        if not self.selected_photo_file_path:
            messagebox.showerror("Ошибка валидации", "Выберите фотографию пользователя!")
            return
        ...
        try:
            # Transactional операция добавления пользователя
            ...
            if self.db.add_user(user_id, name, photo_destination, face_encoding):
                ...
                # Ошибка добавления (дублирование ID) - rollback файловых операций
                audit = self.audit_logger_provider() if self.audit_logger_provider else None
                if audit:
    ...
    def handle_user_photo_update(self):
        ...
        с сохранением целостности данных и proper cleanup старых файлов.
        """
    ...
    def handle_user_deletion(self):
        ...
        Выполняет cascade удаление пользователя с очисткой всех связанных данных:
